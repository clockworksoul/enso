...
**WP-3 SAME-DAY CAPTURE BAR STRATIFIED (2026-07-18, Dross Hour) — the irreducible floor is ~3, not 27:** the Jul-14/15 gate docs treated the same-day supersessions as one monolithic irreducible block (same `LastRefTime` → decay powerless → edge-only). True about *decay*, NOT about *capture*. Stratified the 29 same-day pairs by **edge-free detectability**: **A status-completion=8** (current gains ✅/done/merged), **B explicit-retraction=2** ("previous rule retired", "postponed from"), **C scalar-flip=16** (changed number/date/day/path), **D reword/rescope=3** (equally-specific prose, genuinely no lexical cue → edge-only). **Headline: only ~3 of 29 are genuinely irreducible; ~26 carry an edge-free signal a resolver-side nomination could use** (exactly the deleted-Jul-8 contradiction-check class, already proven 0/18-FP-precise Jul-2). Also **reconciled the 27-vs-29 discrepancy**: 29 total same-day pairs, 27 need the edge, **2 are already won by specificity** (the Adam/Ed out-specifies-stale shape) — that's the gap the earlier docs glossed. Load-bearing for the gate: when capture unblocks (still ADR-001-deferred, blocked on real material-recall telemetry per Jul-16), its **minimal scope is tiers A→B→C in order**, with the ~3 D-cases as known-blind edge-only regression targets — NOT a reflexive "recognize all 29 same-day flips" layer. Bench-only: `wp3_sameday_stratification_test.go` pins the distribution + a `maxIrreducible=8` floor guard so corpus growth that balloons the irreducible class gets caught. Zero production change; `make check` IN SYNC (6 sources), no bench regression. Doc: `docs/2026-07-18-wp3-sameday-capture-bar-stratification.md`.
