...
//
// RESTORED 2026-07-18 (WP-6, ADR-001 corollary b′ / RH-9): deleted in
// `cd8e1a2`, returns with the detection layer per the prescribed route (real
// misses first, after WP-4's gate). Restored verbatim except for plumbing:
// references to the deleted `Entry.Correct` / `confirm` path now point at the
// surviving supersession ceremony and `ProposeSupersession` (propose.go).
//
// # The gap this closes
//
...
// executing the supersession ceremony. The contradiction check only *finds a
// target the bare lexical detector could not*; the committed path stays the
// single chokepoint.
//
...
// dependency — it takes an utterance and an already-loaded candidate Entry.
// ProposeSupersession calls this over the current corpus, but the contradiction
// judgment itself is domain math and belongs with DetectCorrection in core,
// reusing the same Tokenize primitive.
//
...
// ToDetection adapts a Contradiction into the Detection shape the rest of the
// loop already consumes, so a resolver-side contradiction feeds the SAME
// confirm-then-ceremony path as a lexical detection with no parallel code
// path.
//
...
// the operator must supply the new content when confirming. This keeps the
// riskiest inference (auto-writing invented content) barred from the unattended
// path, consistent with detect-don't-decide.
